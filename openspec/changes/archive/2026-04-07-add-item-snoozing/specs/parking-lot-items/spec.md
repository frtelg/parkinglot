...
#### Scenario: Snooze an active item
- **WHEN** a user snoozes an active item until a future time
- **THEN** the system stores the snooze period, removes the item from the Active overview, and shows it in the Snoozed overview until the snooze expires

#### Scenario: Expired snooze returns item to active work
- **WHEN** a snoozed item's wake-up time has passed and the user next loads the item or an overview containing it
- **THEN** the system clears the snooze state and shows the item in the Active overview again without requiring a manual restore action
...
- **WHEN** the user switches between Active, Snoozed, Resolved, and Archived views
...
The system SHALL present item states and actions in a way that keeps the list visually primary, reduces unnecessary interface noise, preserves a user-controlled working order for active items, makes the add-item entry point clearly distinguishable from item rows, and keeps temporarily paused work separate from currently actionable work.
